...
class ToTimeSeries(object):
    ...
    def __call__(self, dataframe):
        ...
        rolling_windows = dataframe["demand"].rolling(window=self.window_size, step=self.step)

        windows = [{"yearly_demand":list(window)}
        for window in rolling_windows if len(window) == self.window_size]
...
def process_dataset(batch_size: int, window_size: int):
    ...
    dataset = PowerDemandDataset(dataframe, transform=ToTimeSeries(window_size=window_size, step=window_size//100))
    train_dataset = PowerDemandDataset(train_dataframe, transform=ToTimeSeries(window_size=window_size, step=window_size//10))
    test_dataset = PowerDemandDataset(test_dataframe, transform=ToTimeSeries(window_size=window_size, step=window_size))
...
def evaluate_model(model: AutoformerForPrediction, test_loader: DataLoader, writer: SummaryWriter):
    ...
    with torch.no_grad():
        for i, batch in enumerate(test_loader):
            ...
            classifier_dataset.append((outputs.loss.item(), labels.item()))
    ...
    return np.array(classifier_dataset)

def train_classifiers(classfier_dataset: Dataset):
    
    clf_data_train, clf_data_test =  train_test_split(classifier_dataset, shuffle=True)

    train_losses = clf_data_train[:, 0].reshape(-1,1)
    train_labels = clf_data_train[:, 1]

    test_losses = clf_data_test[:, 0].reshape(-1,1)
    test_labels = clf_data_test[:, 1]

    svc_clf = SVC(gamma='auto')
    svc_clf.fit(X=train_losses, y=train_labels)

    decision_tree_clf = DecisionTreeClassifier()
    decision_tree_clf.fit(X=train_losses, y=train_labels)

    prediction_svc = svc_clf.predict(test_losses)
    prediction_decision_tree = decision_tree_clf.predict(test_losses)

    dump(decision_tree_clf, "decision_tree_clf.joblib")

    print("--"*10)
    print("Evaluating Classifiers")
    print(f"Support vector machine score: {classification_report(test_labels, prediction_svc)}")
    print(f"Decision Tree classifier score: {classification_report(test_labels, prediction_decision_tree)}")
...
if __name__ == "__main__":
    ...
    distribution_output = hyperparams["distribution_output"]
    model_dimension = hyperparams["model_dimension"]
    ...
    configuration = AutoformerConfig(prediction_length=730, context_length=2183, num_time_features=1, distribution_output=distribution_output, model_d=model_dimension)
...
#
    ...
    classifier_dataset = evaluate_model(model, test_dataloader, writer)

    train_classifiers(classifier_dataset)
